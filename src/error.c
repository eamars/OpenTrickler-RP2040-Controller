...
// Only include u8g2 display code for Mini 12864 builds
#ifndef USE_COLOR_TFT
#include "display.h"
#endif
...
#ifndef USE_COLOR_TFT
// Display layout constants (Mini 12864 only)
...
const char* error_code_to_string(error_code_t code) {
    switch (code) {
        ...
        case ERR_DISPLAY_ENCODER_QUEUE_CREATE: return "Encoder queue";
        ...
        case ERR_WIFI_DNS_BIND_FAIL: return "DNS bind fail";
        ...
        case ERR_MOTOR_INVALID_SELECT: return "Invalid motor";
        ...
        case ERR_MEMORY_STACK_OVERFLOW: return "Stack overflow";
        case ERR_MEMORY_MALLOC_FAILED: return "Malloc failed";
...
#ifndef USE_COLOR_TFT
// Update display with scrolling error list (Mini 12864 only)
...
void report_error(error_code_t code) {
    ...
    if (error_state.neopixel_ready) {
        ...
        // Use mutex-protected public API instead of raw _neopixel_led_set_colour
        rgbw_u32_t error_colour = { ._raw_colour = ERROR_LED_COLOR };
        neopixel_led_set_colour(error_colour, error_colour, error_colour, false);
    }

    // 3. Update display with scrolling error list (Mini 12864 only)
#ifndef USE_COLOR_TFT
    error_update_display();
#endif
    // For TFT displays, errors are logged to serial only for now
    // LVGL-based error display can be added later if needed
